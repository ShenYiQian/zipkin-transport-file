...
module.exports = class FileLogger {
  constructor(options) {
    ...
    this.strEncoder = (span) => {
      ...
      let encode = `{"traceId":"${span.traceId}","parentId":"${span.parentId || ''}","id":"${span.id}","name":"${span.name || ''}","timestamp":${span.timestamp},"duration":${span.duration},"localEndpoint":{"serviceName":"${span.localEndpoint.serviceName || ''}","ipv4":"${span.localEndpoint.ipv4 || ''}","port":${span.localEndpoint.port || ''}},"remoteEndpoint":{"serviceName":"${span.remoteEndpoint.serviceName || ''}","ipv4":"${span.remoteEndpoint.ipv4 || ''}","port":${span.remoteEndpoint.port || ''}},"kind":"${span.kind}"`;
